...
- [PR #1800](https://github.com/nf-core/rnaseq/pull/1800) - Fix per-sample MultiQC stalling in `--skip_quantification_merge` mode: each sample's per-sample MultiQC now closes as soon as its expected files arrive (via pass/fail-aware `groupKey`) rather than waiting for the slowest sample; RUSTQC outputs are bundled per sample so the runtime filename filter no longer drives grouping ([#1797](https://github.com/nf-core/rnaseq/issues/1797))
